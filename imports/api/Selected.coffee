...
exports.selectedContainer = createContainer ((props) ->
  ...
  newProps.options = []
  user = props.user
  points = props[props.type]
  directed = typeof points is 'string' and points.length > 5
  directedTo = typeof props.to is 'string' and props.to.length > 5
  console.log points, directed
  if !directedTo and user?.toLast?
    newProps.to = user.toLast
  if directedTo
    defaultTarget = user[props.type+'Last']
  ...
  console.log directedTo, defaultTarget?, props.type, props.options.length, 'user.fromLast',user.fromLast?, 'user.toLast',user?.toLast?,props, newProps.value?
  props
), Selected
