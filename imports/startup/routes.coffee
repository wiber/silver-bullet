...
FlowRouter.route '/about',
  ...
  action: (params, queryParams) ->
    ...
    if Meteor.userId() and Meteor.isClient and typeof Meteor.user().services is 'object'
      ...
      Meteor.call "Linking"
      , queryParams.from, Meteor.user().services.facebook.email, (error, result) ->
        if error
          console.log "error", error
        #if result
        console.log linkstate.sortByKeysTime(Meteor.user().out[Meteor.userId()])[0], queryParams.from, 'Linked From'
